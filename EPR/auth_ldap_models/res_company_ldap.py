...
class CompanyLDAP(models.Model):
    ...
    def _get_entry(self, conf, login):
        ...
        if len(results) == 1:
            ...
            _logger.warning("LDAP filter %r contains no placeholder ('%%s').", entry)
        return dn, entry
    ...
    def _map_ldap_attributes(self, conf, login, ldap_entry):
        ...
        return {
            ...
            #'department': ldap_entry[1]['ou'][0], #add by kuang
            #'mail': ldap_entry[1]['mail'][0], #add by kuang
            #'uid_number': ldap_entry[1]['uidNumber'][0], #add by kuang
            'login': login,
