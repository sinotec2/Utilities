...
class MaintenanceRequest(models.Model):
    ...
    request_date = fields.Datetime('Request Date', tracking=True, default=fields.Datetime.now(), #.context_today,
    ...
    def _compute_duration(self):
        for record in self:
            if record.close_date and record.schedule_date:
                # 计算时间差并转换为小时
                duration = (record.close_date - record.schedule_date).total_seconds() / 3600.0
                record.duration = duration
            else:
                record.duration = 0.0  # 如果没有日期则设置为0

    duration = fields.Float(compute='_compute_duration', help="Duration in hours.", store=True)
    ...
    def create(self, vals_list):
        ...
        for request in maintenance_requests:
            ...
            if not request.close_date and request.stage_id.done:
                request.close_date = fields.Datetime.now() #.Date.today()
    ...
    def write(self, vals):
        ...
        if vals.get('owner_user_id') or vals.get('user_id'):
            self._add_followers()
        if 'stage_id' in vals:
            self.filtered(lambda m: m.stage_id.done).write({'close_date': fields.Datetime.now()}) #.today()})
